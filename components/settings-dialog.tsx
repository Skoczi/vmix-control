...
export function SettingsDialog(p:Props){
 const {t}=useLanguage(),addressRef=useRef<HTMLInputElement>(null);
 const locked=p.busy||p.connecting;
 return <Dialog open={p.open} onOpenChange={p.onOpenChange}><DialogContent id="connection-settings" className="settings-dialog" overlayClassName="settings-backdrop" showCloseButton={false} initialFocus={addressRef} finalFocus={p.triggerRef}>
 <div className="settings-dialog-heading"><span className="settings-dialog-symbol"><Radio size={23}/></span><div><DialogTitle>{t('Ustawienia')}</DialogTitle><DialogDescription>vMix Control</DialogDescription></div><DialogClose render={<Button variant="ghost"/>} className="settings-dialog-close" aria-label={t('Zamknij')}><X size={18}/></DialogClose></div>
