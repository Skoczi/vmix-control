...
type Props={state:ProgramState;inputs:Source[];locked:boolean;online:boolean;transition:string;stingers:number;onStinger:(effect:string)=>void;onControl:(control:ProgramControl)=>void};
export function ProgramPanel(p:Props){
 const {t}=useLanguage();
 const [selected,setSelected]=useState<Record<number,string>>({});
 const [armed,setArmed]=useState(false);
 const locked=p.locked||!p.online;
 return <section className="program-tools" aria-label={t('PGM controls')}>
  <div className="program-tools-top"><div><span className="program-tools-label">PGM</span><strong>{t('Production controls')}</strong></div><div className="program-output-status">{([['REC',p.state.recording],['STREAM',p.state.streaming],['EXT',p.state.external]] as const).map(([label,value])=><span key={label} data-active={p.online&&value===true} title={!p.online?t('Brak połączenia'):value===null?'—':value?'ON':'OFF'}><i/>{label}</span>)}</div></div>
  ...
  {!!Object.keys(p.state.overlays).length&&<details className="program-overlays" open><summary><Layers size={14}/><span>OVERLAYS</span><small>{t('Shared channels')}</small><ChevronDown size={14}/></summary><div className="program-overlay-grid">{Object.entries(p.state.overlays).map(([number,value])=>{
   ...
   const valid=p.inputs.some(i=>i.key===choice);
   return <div className="program-overlay" key={channel} data-active={p.online&&!!value}><div className="program-overlay-heading"><span>OVL {number.padStart(2,'0')}</span><i>{!p.online?'—':value?'ON':'OFF'}</i></div><NativeSelect aria-label={`Overlay ${number} · ${t('Wybierz źródło')}`} value={valid?choice:''} disabled={locked} onChange={e=>setSelected(prev=>({...prev,[channel]:e.target.value}))}><NativeSelectOption value="">{t('Wybierz źródło')}</NativeSelectOption>{p.inputs.map(input=><NativeSelectOption key={input.key} value={input.key}>{input.number} · {input.title}</NativeSelectOption>)}</NativeSelect><div className="program-overlay-actions"><span title={active?.title}>{active?.title||'—'}</span><Button variant="outline" disabled={locked||!valid||active?.key===choice} onClick={()=>p.onControl({kind:'overlay',channel,enabled:true,input:choice,expected:active?.key||''})}>IN</Button><Button variant="outline" disabled={locked||!active} title={t('OUT clears this shared channel on every mix.')} onClick={()=>p.onControl({kind:'overlay',channel,enabled:false,expected:active?.key||''})}>OUT</Button></div></div>;
  })}</div></details>}
 </section>;
}
