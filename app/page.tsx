...
function parse(xml: string): Snapshot {
  ...
  return { program:readProgramState(xml), inputs, mixes, previews, mixInfo: mixInformation(inputs, mixes, doc.querySelector('version')?.textContent==='DEMO'?'PGM':'Program główny'), version: doc.querySelector('version')?.textContent || '' };
}
...
function Dashboard() {
 ...
 const {t,language}=useLanguage();
  ...
  const [savedOperatorName,setOperatorName]=useState(loginOperatorName);
  const operatorName=canConfigure?'Host (Admin)':savedOperatorName;
  ...
  function changeOperatorName(value:string){if(canConfigure)return;setOperatorName(value);try{localStorage.setItem('vmix-operator-name',value);}catch{}}
  ...
  async function api(host: string, command?: { input?: string; control?:ProgramControl; mix: number; play: boolean; mixId: string; transition: string; duration: number; action?: string; expectedProgram?:string }, announce=false) {
  ...
  return <main className={`${singleMode ? 'console-mode' : 'matrix-mode'} tile-size-${tileSize} ${focusMode?'focus-mode':''} ${onAir?'on-air-mode':''}`}>
    ...
    <div className="workspace-toolbar">
      <section className="operator-panel"><MixPicker value={selectedMix} onChange={selectMix} disabled={busy||connecting||onAir} loaded={!!snapshot} mixes={availableMixNumbers.map(n=>({...snapshot!.mixInfo[n],name:mixDisplayName(n),number:n}))}/>{singleMode&&<div className="view-switch" aria-label={t("Tryb panelu")}><Button variant="outline" disabled={busy||onAir} aria-pressed={director} onClick={()=>toggleDirector(true)}>Switcher</Button><Button variant="outline" disabled={busy||onAir} aria-pressed={!director} onClick={()=>toggleDirector(false)}>{t("Źródła")}</Button></div>}</section>
      <span className="focus-mix-name">{singleMode?mixDisplayName(focusedMix):t("Mix niedostępny")}</span>
      <div className="workspace-toolbar-actions">{online&&<Popover><PopoverTrigger render={<Button variant="outline"/>} className="presence-trigger" aria-label={`${t('Operatorzy')}: ${presenceAvailable?operators.length+1:t('Obecność niedostępna')}`} title={t('Operatorzy')}><Users size={15}/><span>{presenceAvailable?operators.length+1:'—'}</span></PopoverTrigger><PopoverContent className="presence-popup" align="end" sideOffset={8}><PopoverTitle>{t('Operatorzy')}</PopoverTitle><div className="presence-list"><span className="presence-self">{operatorName||t('Operator')} · {selectedMix==='all'?t('Wszystkie mixy'):focusedMix?mixTitle(focusedMix):t('Mix niedostępny')}</span>{!presenceAvailable?<span>{t('Obecność niedostępna')}</span>:operators.map(op=><span key={op.id} className={op.mixId===selectedMix?'same-mix':''}>{op.name||t('Operator')} · {op.mixId==='all'?t('Wszystkie mixy'):snapshot?.mixInfo[availableMixNumbers.find(n=>snapshot?.mixInfo[n].id===op.mixId)||0]?.name||t('Mix niedostępny')}{op.onAir?' · ON AIR':''}</span>)}</div></PopoverContent></Popover>}{(singleMode||onAir||focusMode)&&<div className="operations-bar"><div className="operations-actions"><Button variant="outline" className="return-source" disabled={!singleMode||!online||busy||!previousSource||previousSource.key===programInput?.key} title={previousSource?.title} onClick={()=>{if(previousSource)void route(previousSource,focusedMix,'back','Cut');}}><Undo2 size={15}/><span>{t('Powrót')}{previousSource?` · ${previousSource.title}`:''}</span></Button><Button variant="outline" aria-pressed={onAir} title={t("Blokada konfiguracji")} disabled={busy||(!online&&!onAir)} onClick={()=>{setOnAir(!onAir);setSettingsOpen(false);setStationOpen(false);}}>{onAir?<LockKeyhole size={14}/>:<Unlock size={14}/>}ON AIR</Button><Button variant="outline" aria-pressed={focusMode} onClick={()=>{setFocusMode(!focusMode);if(!focusMode)setDirector(true);setStationOpen(false);}}><Scan size={15}/>{t(focusMode?'Wyjdź ze skupienia':'Skupienie')}</Button></div></div>}</div>
    </div>
    <div id="station-settings" hidden={!stationOpen}><StationPanel name={stationName} setName={setStationName} profiles={profiles} onProfilesChange={changeProfiles} onRename={(old,next)=>{if(stationName===old)setStationName(next);}} onLoad={applyProfile} onSave={saveProfile} onShare={() => void shareProfile()} link={stationLink} layout={layout} onLayout={updateLayout} inputs={snapshot?.inputs||[]} operatorName={operatorName} operatorNameReadOnly={canConfigure} onOperatorName={changeOperatorName} disabled={busy||connecting||onAir} canSave={!!snapshot && selectedMix!=='all' && !missingSelection && durationValid}/></div>
