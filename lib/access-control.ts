...
export function createAccessControl(file=path.resolve(process.env.VMIX_ACCESS_FILE||'.vmix-access.json'),now=Date.now,canConfigure:(request:Request)=>boolean=isLocalRequest){
 const sessions=new Map<string,{expires:number;operatorName:string}>();let locked=false;let failures=0;let retryAt=0;
 ...
 function authenticated(request:Request){for(const [key,value] of sessions)if(value.expires<=now())sessions.delete(key);return isLocalRequest(request)||sessions.has(token(request));}
 ...
 function session(request:Request,operatorName=''){if(sessions.size>=256)sessions.delete(sessions.keys().next().value!);const value=randomBytes(32).toString('hex');sessions.set(value,{expires:now()+8*60*60*1000,operatorName});return cookie(request,value,8*60*60);}
 ...
 async function handle(request:Request){
  try{
   ...
   if(request.method==='GET'){const allowed=!c||authenticated(request);return json({enabled:!!c,authenticated:allowed,local:isLocalRequest(request),operatorName:allowed&&c&&!isLocalRequest(request)?sessions.get(token(request))?.operatorName||'':''});}
   ...
   if(body.action==='login'&&(typeof body.operatorName!=='string'||!body.operatorName.trim()||body.operatorName.length>80))return json({error:'Enter an operator name (1–80 characters)'},400);
   ...
   try{
    ...
    if(body.action==='login'){failures=0;return json({ok:true},200,c?session(request,body.operatorName.trim()):undefined);}
